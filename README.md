...
- **The mark — "Signal":** a tile and one square of accent, the way the dashboard itself uses colour: nothing but the signal. It is the app's own mark, not Strava's chevron — this reads Strava, it is not Strava, and the chevron now appears only on the one row that actually links there. One mark in three places: the favicon (an inline data URI in `<head>`), the logo in the top left (`.logo-icon` + `.logo-square`), and the app icons. All three are built from `icons/icon.svg`'s own geometry — a 143 square at 287 of 512 — so the square lands in the same spot at every size, and a smoke check measures both the tab's and the header's to make sure they have not drifted apart.
- **The tile inverts against its ground; it does not match it.** Ink on a light theme, paper on a dark one (`--logo-tile`), with the accent square constant at `#ff385c` in both. Drawn the other way round — a tile the colour of the page, with a hairline to delineate it — the mark dissolves into a white browser tab or the app's own white header and leaves a pink dot floating in nothing, which at 16px is no mark at all. That is why the hairline frame is gone from the favicon and the header: it only ever existed to rescue a tile that matched its ground.
- **App icon:** the same mark, always inked — a home-screen icon sits on wallpaper, not on a page, so it has no theme to invert against. `icons/icon.svg` is the source and `node icons/render.mjs` cuts the PNGs from the same geometry, the maskable one with the square pulled in toward the centre to survive a circular mask.
