...
- **Favicon:** the Strava chevron on Strava's orange (`#fc4c02`), the same mark the header wears in the top left, so the tab and the page agree about what this is. It is an inline data URI in `<head>`, centred in its tile at 72% — big enough to read at 16px, short of the edges so it still looks like a tile rather than a crop — and it does not follow the system theme, because the orange is the identity in either palette. Note this is Strava's trademark, used here on a personal dashboard; a public, branded deployment would want its own mark back.
- **App icon:** "Signal" — an ink tile with one square of accent, the dashboard's own mark. The SVG follows the system theme (paper with a hairline in light, ink in dark); `icons/icon.svg` is the source and `node icons/render.mjs` cuts the PNGs from the same geometry, the maskable one with the square pulled in to survive a circular mask. The home-screen icon and the favicon are deliberately different marks — changing one does not change the other.
