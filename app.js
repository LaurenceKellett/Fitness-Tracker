...
/* ── THE SAME LAYOUT ON EVERY DEVICE ──
 * The order is held in two places. localStorage is what renders: it is
 * synchronous, it survives being offline, and the page must never wait on a
 * network round trip to draw its panels in the right order. The Worker holds the
 * other copy, under one KV key, and that is the one that carries an arrangement
 * made on the laptop over to the phone.
 *
 * Both copies carry a millisecond stamp and the newer wins outright. There is no
 * merge to attempt: the thing being synced is a single ordering per zone, so two
 * devices disagreeing means the more recent arrangement is the one that was meant.
 *
 * Everything here fails silently and on purpose. A dead network, a cold Worker or
 * a browser with storage switched off has to leave a working dashboard behind —
 * in the local order if there is one, the default order if there is not.
 */
const LAYOUT_STAMP_KEY='fitness_layout_at_v1';
let prefsPushTimer=null;

// A function rather than a constant: API_BASE is declared further down this same
// script, and reading it up here at parse time would be a temporal-dead-zone error
// before the page had drawn anything at all.
function prefsUrl(){return API_BASE+'/prefs';}

function layoutStamp(){
  try{
    const n=+(localStorage.getItem(LAYOUT_STAMP_KEY)||0);
    return Number.isFinite(n)?n:0;
  }catch(e){return 0;}
}
function stampLayout(){
  try{localStorage.setItem(LAYOUT_STAMP_KEY,String(Date.now()));}catch(e){}
}

// Debounced. Dragging fires saveLayout on every drop and the keyboard path fires it
// on every arrow press; one write a second after you stop is plenty.
function queuePrefsPush(){
  clearTimeout(prefsPushTimer);
  prefsPushTimer=setTimeout(pushPrefs,1000);
}

// readLayout()||{} rather than a bail-out on empty: an order that has been reset
// away is still a change, and has to be sent as one.
function pushPrefs(){
  try{
    fetch(prefsUrl(),{
      method:'PUT',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({updatedAt:layoutStamp(),prefs:{layout:readLayout()||{}}}),
    }).catch(()=>{});
  }catch(e){}
}

// At boot, once. If the server's copy is newer the tabs are re-laid out in place
// rather than waiting for whatever redraws them next; if ours is newer it goes up,
// which is also how a first run seeds an empty server.
function pullPrefs(){
  try{
    fetch(prefsUrl())
      ...
      .then(rec=>{
        if(!rec)return;
        const remoteAt=Number(rec.updatedAt)||0;
        const localAt=layoutStamp();
        const remote=rec.prefs&&rec.prefs.layout;
        if(remoteAt>localAt&&remote&&typeof remote==='object'&&!Array.isArray(remote)){
          try{
            localStorage.setItem(LAYOUT_KEY,JSON.stringify(remote));
            localStorage.setItem(LAYOUT_STAMP_KEY,String(remoteAt));
          }catch(e){}
          restoreAllLayouts();
          // Reordering can be open on a slow connection, and its move buttons are
          // built against the order they were drawn from.
          if(reorderOn)buildReorderControls();
        }else if(readLayout()&&(localAt>remoteAt||remoteAt===0)){
          // A layout saved before syncing existed has no stamp. Against an empty
          // server it is still the only arrangement there is, so it is stamped now
          // and sent up — otherwise the laptop's order would sit unshared until
          // the first time something on it was moved.
          if(!localAt)stampLayout();
          pushPrefs();
        }
      })
      .catch(()=>{});
...
function resetLayout(){
  ...
  // A reset is a change like any other, and has to travel: without this, clearing
  // the order here would be undone by the next device that pushed its own.
  stampLayout();
  queuePrefsPush();
...
// How far back the Consistency calendar looks. Twelve weeks: see the grid note at
// the call site for why it is not thirteen.
...
function renderSummaryNow(data){
  ...
  // "This week" means the week you are standing in — Monday to today — and not a
  // rolling seven days. The chart below stays rolling and stays labelled that way;
  // this figure is the one that ought to read zero on a Monday morning, because it
  // does. See calendarWeek in calc.js for why the base is a part-week too.
  const cw=calendarWeek(data,a=>(a.mt||0)/3600,{today});
  const ratio=cw.ratio;

  // ── Hero figure ──
  const bigEl=document.getElementById('sumWeekHours');
  if(bigEl)bigEl.innerHTML=`${cw.total.toFixed(1)}<span class="sum-big-unit">hours</span>`;
  const baseEl=document.getElementById('sumWeekBase');
  if(baseEl)baseEl.textContent=weekBaseText(cw,'h');
  const ratioEl=document.getElementById('sumWeekRatio');
  if(ratioEl){
    if(!(cw.pace>0)||!(cw.total>0)){ratioEl.innerHTML='';}
    else{
  ...
  // ── Consistency: days trained out of the last twelve weeks, plus the streaks ──
  // Twelve weeks rather than thirteen so the grid stays 14 wide and six deep with
  // no ragged last row — and because 14 is two whole weeks, every column stays one
  // weekday, so a day you never train reads as a vertical stripe.
  const trained=new Set(data.map(a=>a.date));
  const cells=[];
  for(let k=CONSISTENCY_DAYS-1;k>=0;k--)cells.push(trained.has(isoOf(endMs-k*86400000)));
  ...
  if(consEl)consEl.innerHTML=
    `<div class="stat-value" style="font-size:34px;margin-top:12px">${active}`+
      `<span class="stat-unit" style="font-size:15px">of ${CONSISTENCY_DAYS} days</span></div>`+
    `<div class="chart-sub" style="margin:6px 0 0">You trained on ${Math.round(active/CONSISTENCY_DAYS*100)}% of them</div>`+
    `<div class="sum-cal">${cells.map(x=>
      `<i style="background:${x?getAccentColor():'var(--border)'}"></i>`).join('')}</div>`+
    `<div class="sum-cal-ends"><span>12 weeks ago</span><span>today</span></div>`+
  ...
  groups.forEach(g=>{
    ...
    // Same window as the hero: this calendar week, against the same slice of the
    // last four. A part-week held against a full-week base would mark every sport
    // as "missing" until about Thursday.
    const cwH=calendarWeek(all,a=>(a.mt||0)/3600,{today});
    const wk=cwH.total;
    const base=cwH.pace;
    const wkDist=calendarWeek(all,a=>(a.dist_mi||0),{today}).total;
  ...
  if(badge)badge.textContent='Week of '+fmtDate(cw.weekStart);
  ...
  if(wrap)wrap.innerHTML=cards.slice(0,5).map(c=>{
    ...
    return`<div class="sum-sport-card" style="border-top-color:${col}">
      ...
      <div class="sum-sport-rows">
        <div class="sum-sport-row"><span>by now, 4-wk avg</span><span>${c.base.toFixed(1)}h</span></div>
  ...
  if(band){
    ...
    if(!(cw.pace>0)){band.innerHTML='';}
    else{
      const state=!(cw.total>0)?'yet to start'
        :ratio>1.3?'building':ratio<0.8?'backing off':'holding steady';
      ...
      const sportBit=(cw.total>0&&moved&&moved.wk<moved.base*0.7)
      ...
      band.innerHTML=`<span class="ms">insights</span><div class="sum-band-text">You are `+
        `<span style="color:${stateCol}">${state}</span> — ${cw.total.toFixed(1)} hours so far this week `+
        `against ${cw.pace.toFixed(1)} by the same point in an average week${yearBit}.${sportBit}</div>`;
    }
  }
}
...
function renderLoad(data,t=LOAD_TARGET){
  ...
  (()=>{
    ...
    if(ratio>1.5)return `The last seven days are ${ratio.toFixed(1)}× your 28-day base. Spells that far above the line are where injuries come from — the number worth watching is not the total, it is the jump.`;
    if(ratio>1.3)return `Building: the last seven days are ${Math.round((ratio-1)*100)}% above your 28-day base. That is a real step up, and a sustainable one if it does not repeat.`;
    if(ratio<0.6)return `The last seven days are ${Math.round((1-ratio)*100)}% below your base. Fine as a taper or a rest week, worth noticing if it was not planned.`;
    if(ratio<0.8)return `Backing off slightly — ${a7.toFixed(1)}h against a ${c28.toFixed(1)}h base.`;
    return `Steady: ${a7.toFixed(1)}h over the last seven days against a ${c28.toFixed(1)}h base. Neither building nor letting go.`;
  })());
}
...
// How the hero figures describe what they are measured against. The base is the
// SAME SLICE of the preceding four weeks, so the wording has to say so: "by this
// point" is the whole difference between a fair comparison and a part-week held
// against a whole one. Serves hours and distance, hence the unit being passed in.
function weekBaseText(cw,unitWord){
  if(!(cw.pace>0))return 'nothing in the last four weeks to compare this week against';
  const n=cw.pace<100?cw.pace.toFixed(1):Math.round(cw.pace);
  return cw.complete
    ? `against ${n} ${unitWord} in an average week over the last four`
    : `against ${n} ${unitWord} by this point in an average week`;
}

// Distance by sport group across one date range, biggest first. Two windows need
// this now — the calendar week under the figure, and the rolling seven days the
// chart's own verdict talks about — and they must not quietly share one total.
function sportSplit(data,fromISO,toISO){
  const by={};
  data.forEach(a=>{
    if(a.date>=fromISO&&a.date<=toISO){const g=typeGroup(a.type);by[g]=(by[g]||0)+actDistIn(a);}
  });
  return Object.entries(by).filter(([,d])=>d>0).sort((x,y)=>y[1]-x[1]);
}
...
function renderWeekDistance(data,t=SUM_DIST_TARGET){
  ...
  const a7=w.latest.acute,c28=w.latest.chronic;
  const ratio=c28>0?a7/c28:0;
  // The figure is the calendar week, matching the hours card above it. The chart
  // and the chips under it stay rolling, and stay labelled "last 7 days".
  const cw=calendarWeek(data,a=>actDistIn(a),{today:_today()});

  setFig(
    chipNum(cw.total,cw.total<100?1:0),
    weekBaseText(cw,u),
    cw.pace>0&&cw.total>0?(()=>{
      const[col,word]=ratioTone(cw.ratio);
      return `<div class="sum-ratio" style="color:${col};background:${col}1a"><strong>${cw.ratio.toFixed(2)}×</strong> ${word}</div>`;
    })():''
  );
  ...
  // ── This week, by sport ──
  // The calendar week, so it adds up to the figure above rather than to the chart.
  const split=sportSplit(data,cw.weekStart,_today());
  renderWeekSplit(split,cw.total);
  // The verdict belongs to the chips beneath it, which are the rolling seven days,
  // so it gets its own split over that window. Sharing one would have the verdict
  // quoting percentages of a total it is not describing.
  const rollSplit=sportSplit(data,isoOf(new Date(_today()+'T12:00:00').getTime()-6*86400000),_today());
  ...
  (()=>{
    ...
    const top=rollSplit[0];
    const share=top?Math.round(top[1]/a7*100):0;
    const mix=top&&rollSplit.length>1
      ? ` ${top[0]} carried ${share}% of it, across ${rollSplit.length} sports.`
...
function renderWeekSplit(split,total){
  ...
  if(!split.length||!(total>0)){
    el.innerHTML=`<div class="sum-split"><div class="sum-split-head"><span class="sum-split-t">This week by sport</span></div>`+
      `<div class="sum-split-empty">Nothing logged yet this week.</div></div>`;
    return;
  }
...
restoreAllLayouts();
// After the local order is on screen, never before it: the network decides only
// whether to replace what is already drawn.
pullPrefs();
